...
import { Check, ChevronRight, ExternalLink, Filter, MailCheck, Route, Save, Send, Sparkles, Users } from "lucide-react";
import clsx from "clsx";
import { api } from "../../convex/_generated/api";
import type { Doc } from "../../convex/_generated/dataModel";
import { buildGmailComposeUrl, matchRoutingRule, personalizeTemplate } from "../domain/email";
...
const DEFAULT_BODY = `<p>Hello {{firstName}},</p><p>We’re writing with an update about your Oakridge MUN registration.</p><p><strong>Committee:</strong> {{committee}}<br><strong>Allocation:</strong> {{allocation}}</p><p>If you have any questions, reply to this email and we’ll help.</p><p>Best,<br>Oakridge MUN Team</p>`;
const mergeFields = [
...
function contactFields(contact: Doc<"contacts">) {
  return {
    ...
    committee: contact.assignedCommittee ?? "",
...
export function EmailPage() {
  ...
  const [subject, setSubject] = useState("Your Oakridge MUN allocation");
  ...
  return (
    <div className="page">
      <PageHeader eyebrow="Email studio" title="Write once. Make it personal." description={`Outbound drafts use ${SENDER}. Nothing sends without your review.`} />
      ...
      {tab === "write" && (
        <div className="email-studio">
          ...
          <section className="composer-panel">
            ...
            <div className="composer-footer"><div><strong>{selected.size || 0} personalized draft{selected.size === 1 ? "" : "s"}</strong><small>Each person gets their own copy.</small></div><button className="button button--primary" type="button" disabled={!selected.size || busy || unresolved.length > 0} onClick={() => void prepareDrafts()}><Save aria-hidden="true" /> {busy ? "Preparing…" : "Prepare drafts"}</button></div>
          ...
          <aside className="preview-panel">
            ...
            {previewContact && previewBody && previewSubject ? (
              <div className="email-preview">
                ...
                <div className="preview-body" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(previewBody.output) }} />
                <button className="button button--secondary button--full" type="button" disabled={unresolved.length > 0} onClick={() => openDraft(previewContact)}><ExternalLink aria-hidden="true" /> Open this draft in Gmail</button>
              </div>
